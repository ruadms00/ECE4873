<?php
   include("config.php");
   
   session_start();
   
   if($_SERVER["REQUEST_METHOD"] == "POST") {
      // username and password sent from form 
      
      $mydevice = $conn->real_escape_string($_POST["devid"]);
      $myusername = $conn->real_escape_string($_POST["uname"]);
      $mypassword = $conn->real_escape_string($_POST["password"]); 
      
      
      $sql = "SELECT * FROM users WHERE userid = '$myusername' and password = '$mypassword' and device = '$mydevice'";
      $result = mysqli_query($conn,$sql);
      
      $count = mysqli_num_rows($result);

      // If result matched $myusername and $mypassword, table row must be 1 row
		
      if($count == 1) {
         $_SESSION['login_user'] = $myusername;
         $row = mysqli_fetch_assoc($result);
         if ($row["is_owner"]) {
           ob_start();
           header("Location: ControlAdd.php");
           exit();
         }
         else {
           ob_start();
           header("Location: User.php");
           exit();
         }
      }else {
         $error = "Your Login Name or Password is invalid";
      }
   }
?>

<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>Boombox</title>
    
    <link href='https://fonts.googleapis.com/css?family=Strait' rel='stylesheet'>
    <style type="text/css">
    <!--
    body {
      color:#0080FF;
      font-family: 'Strait';font-size: 22px;
      background-color:#4B4B4B;
      background-image:url('Background Image');
      background-repeat:no-repeat;
    }
    a  { color:#0000FF; }
    a:visited { color:#800080; }
    a:hover { color:#008000; }
    a:active { color:#FF0000; }

	h1 {
	   text-align: center;
	}
	
  input[type=text], input[type=password] {
    width: 100%;
    padding: 12px 20px;
    margin: 8px 0;
    display: inline-block;
    border: 1px solid #ccc;
    box-sizing: border-box;
  }
  
  button {
    background-color: #4CAF50;
    color: white;
    padding: 14px 20px;
    margin: 8px 0;
    border: none;
    cursor: pointer;
    width: 100%;
  }
  
  button:hover {
    opacity: 0.8;
  }
  
  .left {
    padding: 16px;
	width:50%;
	float:left;
  }
  
  .right {
    padding: 16px;
	margin-left:50%;
  }
  
  span.psw {
    float: right;
    padding-top: 16px;
  }
  
  /* Change styles for span and cancel button on extra small screens */
  @media screen and (max-width: 300px) {
    span.psw {
       display: block;
       float: none;
    }
    .cancelbtn {
       width: 100%;
    }
  }
    -->
    </style>
  </head>
  <body>
  <header>
  <h1>
  BOOMBOX OWNER
    </h1>
  </header>
  
<form action = "" method = "post">

  <div class="left">
    <label for="devid"><b>Device</b></label>
    <input type="text" placeholder="Enter Device ID" name="devid" required>
    
    <label for="uname"><b>Username</b></label>
    <input type="text" placeholder="Enter Username" name="uname" required>
    
    <label for="pw"><b>Password</b></label>
    <input type="text" placeholder="Enter Password" name="password" required>

    <button type="submit">Connect</button>
    <label>
      <input type="checkbox" checked="checked" name="rem"> Connect Automatically
    </label>
  </div>
  <div class="right">
 <label>
 OR SCAN QR CODE HERE
 </label>
  </div>
</form>

  </body>
</html>
